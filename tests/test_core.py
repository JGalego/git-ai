...
class TestGitAI:
    ...
    def test_init_command(self, git_repo):
        ...
        config = git_ai._load_config()
        assert config['version'] == '1.0'
        assert 'ai_systems' in config
        assert 'settings' in config
    
    def test_track_ai_system(self, git_repo):
    ...
    def test_git_command_execution(self, git_repo):
        """Test git command execution"""
        git_ai = GitAI()
        
        result = git_ai._run_git_command(['status', '--porcelain'])
